...
def init_wrench(a,i):
  f = open('/home/isaacnewthanh/PyScripts/sample_wrench.json')
  data = json.load(f)
  if i == 0:
    return data[a]["x"]
  if i == 1:
  ...
  if i == 2:
    return data[a]["z"]
def init_img():
  f = open('/home/isaacnewthanh/ImgProcess/yolov5/yolov5/numblock.json')#image proccessing json file
  data = json.load(f)
  return data["num"]

def init_data(a,i):
  path = '/home/isaacnewthanh/catkin_ws/sample2.json' #position and velocity json file
  f = open(path)
  data = json.load(f)
  return data[a][i]
...
class App():
  ...
  def __init__(self, master = None):
    ...
    App.apiUrl.set('https://api-dccs-ensaas.sa.wise-paas.com/')
    ...
    App.credentialKey.set('93cc51d991bb31684b2cedf6473902wi')
    ...
    def __generateData():
      ...
      for i in range(1, int(App.deviceCount.get()) + 1):
        for j in range(1, int(App.analogCount.get()) + 1):
          ...
          try:
            if j<=6:
              value = init_data("position",j-1)
            if j<=12 and j>6:
              value = init_data("velocity",j-7)
            if j==13:
              value = init_img()
            if j<=16 and j>13:
              value = init_wrench("force",j-14)
            if j<=19 and j>16:
              value = init_wrench("torque",j-17)
    ...
    App.nodeId.set('11f2b9e8-2f0d-4e06-989e-9e60b77702ef')
